...
##summary of percent land use in each subbasin
summary_land_use <- land_use_subs %>%
  ...
  unique() %>%
  group_by(noaa_subba, master_cat) %>%
  # Sum the parcel area for each group
  summarise(total_parcel_area = sum(parcel_area)) %>%
  # Ungroup the data
  ungroup() %>%
  # Group by 'noaa_subba' again
  group_by(noaa_subba) %>%
  # Calculate the relative percentage for each 'master_cat' within each 'noaa_subba'
  mutate(perc_land_use = round(total_parcel_area / sum(total_parcel_area) * 100,2))
```
