...
sc_avg_costs <- side_channel_flow %>% 
  ...
  st_drop_geometry() %>% 
  group_by(noaa_subba) %>% 
  summarise(lower_avg_cost = mean(lower_cost, na.rm = TRUE),
            upper_avg_cost = mean(upper_cost, na.rm = TRUE)) %>% 
  mutate(hab_unit="Side Channel") %>% 
  na.omit()
```
...
marsh_avg_costs <- marsh_cost_inflation %>% 
  st_drop_geometry() %>% 
  group_by(noaa_subba, hab_unit) %>% 
  summarise(lower_avg_cost = mean(lower_cost),
            upper_avg_cost = mean(upper_cost))
