...
hist_flood_stl <- filter(flood_stl, period %in% "Hist") %>% 
  st_join(subs_mod, largest = TRUE)  

# mapview::mapview(test, zcol = "noaa_subba")
...
hist_flood_stl_var_mult <- left_join(hist_flood_stl_var, side_channel_mult_stl, 
                                     by = c("noaa_subba"= "reach_name")) %>% 
...
pond_avg_costs <- pond_cost_inflation %>% 
  group_by(noaa_subba, hab_unit) %>% 
  summarize(lower_avg_cost = mean(lower_cost),
            upper_avg_cost = mean(upper_cost))
```
...
side_channel_flow <- side_channel_flow %>% 
  ...
  mutate(area_a=sc_area*0.000247105) %>% #meters squared to actres
  mutate(total_lower_cost=lower_cost*area_a) %>% 
  mutate(total_upper_cost=upper_cost*area_a)
```

# get side channel avg cost/acre
```{r}
sc_avg_costs <- side_channel_flow %>% 
  st_drop_geometry() %>% 
  group_by(noaa_subba) %>% 
  summarise(lower_avg_cost = mean(total_lower_cost, na.rm = TRUE),
            upper_avg_cost = mean(total_upper_cost, na.rm = TRUE)) %>% 
...
side_channel_costs<- side_channel_flow %>%
  ...
  summarize(total_lower_cost = sum(total_lower_cost),
...
marsh_cost <- calculate_marsh_cost(filter(marsh_energy_stl,
                                          hab_unit == "Marsh"))
...
# avg cost/ acre marsh and ponds
```{r}
# get side channel avg cost/acre
marsh_avg_costs <- marsh_cost_inflation %>% 
  st_drop_geometry() %>% 
  group_by(noaa_subba, hab_unit) %>% 
  summarise(lower_avg_cost = sum(lower_cost)/sum(area_a),
            upper_avg_cost = sum(upper_cost)/sum(area_a))
...
#marsh costs
marsh_cost_inflation_select <-
  marsh_cost_inflation %>% 
...
stilly_subbasin_cost <- stilly_total_cost %>% 
  group_by(noaa_subba) %>% 
  summarise(total_lower_cost = sum(total_lower_cost), 
            total_upper_cost = sum(total_upper_cost)) %>%
  st_drop_geometry()

#adding side channel
stilly_subbasin_cost<-full_join(stilly_subbasin_cost, side_channel_inflation_select) %>% 
  st_drop_geometry() %>% 
  group_by(noaa_subba) %>% 
  summarize(total_lower_cost=sum(total_lower_cost), 
    total_upper_cost=sum(total_upper_cost))%>% 
...
# avg costs 
```{r}
avg_fp_costs <- rbind(pond_avg_costs, 
      marsh_avg_costs,
      sc_avg_costs) %>% 
  rename("Subbasin" = noaa_subba) %>% 
  transmute("Restoration" = hab_unit,
            Metric = "per_acre",
         "Lower Average Cost" = 
           paste("$", format(round(lower_avg_cost), big.mark = ",")),
         "Upper Average Cost" = 
           paste("$", format(round(upper_avg_cost), big.mark = ","))) %>% 
  arrange(Subbasin)
...
kableExtra::kable() %>%
   ...
   kableExtra::save_kable(file = here("final_prods","tables","average_fp_costs.png"), zoom = 4)

write.csv(avg_fp_costs, "../final_prods/tables/average_fp_costs.csv")
```
...
```{r}
marsh_pond_area<-full_join(marsh_cost_inflation, pond_cost_inflation) %>% st_drop_geometry()%>% select(hab_unit, area_ha, noaa_subba)%>%
  group_by(noaa_subba, hab_unit) %>%
  summarise(total_area_ha = sum(area_ha))

#pull in fp shapefiles for adjacent to river habitat
side_channel_area<-st_read(here("data", "fp_shapefiles", "sidechannel_shapefiles.shp")) 
#calculate area
side_channel_area$area_ha<-st_area(side_channel_area)/10000
#drop units
side_channel_area$area_ha <- as.numeric(side_channel_area$area_ha)
side_channel_area$hab_unit<-"side_channel"
side_channel_area<-side_channel_area %>% 
  select(hab_unit, area_ha, noa_sbb) %>% 
  rename("noaa_subba"="noa_sbb")%>%
  group_by(noaa_subba, hab_unit) %>%
  summarise(total_area_ha = sum(area_ha)) %>% 
  st_drop_geometry()

fp_area_subbasin<-rbind(marsh_pond_area, side_channel_area)

write.csv(fp_area_subbasin, here("data", "final_dfs", "fp_area_subbasin.csv"))

#summary by restoration_type
fp_area_sum<-full_join(marsh_pond_area, side_channel_area) %>% 
  group_by(hab_unit) %>% summarise(area_ha=sum(total_area_ha)) 
