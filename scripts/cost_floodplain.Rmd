...
test <- side_channel_flow %>% 
filter(noaa_subba == "mainstem_Stillaguamish") %>% 
  summarize(lower = sum(total_lower_cost),
            upper = sum(total_upper_cost))
...
side_channel_area<-side_channel_area %>% select(hab_unit, area_ha, noa_sbb) %>% 
