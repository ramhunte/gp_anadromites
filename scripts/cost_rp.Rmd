...
#assigning energy 
flow_stl_rp <- assign_energy_width(flow_stl_rp, low_slope_range, medium_slope_range, high_slope_range, low_width_range, medium_width_range, high_width_range, slope_degrees)

unique(flow_stl_rp$energy)
```
...
```{r}
#### creating a slope raster file of the stillaguamish, extractign average slope by surround stream reach buffer, and classifying it as low, medium, or high to estimate level of site preparation
###---------------------------------------------------------------------------------------
...
flow_stl_rp <- flow_stl_rp %>%
  mutate(bank_slope_class = bank_slope_cost_fun(avg_bank_slope),
        ...
        can_ang_chng = ((can_ang - hist_ang)/hist_ang)
         )
```
...
rp_cost <- function(input_df) {
  input_df <- input_df %>%
    mutate(
      ...
      lower_cost = rip_acres*can_ang_chng*case_when(
      ...
      #multiply the total number of acres by the cost per acre
      upper_cost = rip_acres*can_ang_chng*case_when(
