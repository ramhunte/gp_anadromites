...
rp_cost_acre_summary <- rp_cost_final %>% 
  ...
  st_drop_geometry() 
  group_by(noaa_subba) %>% 
  summarise(lower_avg = mean(lower_cost_acre_ppi, na.rm = TRUE),
            upper_avge = mean(upper_cost_acre_ppi, na.rm = TRUE))
...
st_write(subba_rp_cost, here("data", "final_dfs", "rp_costs.gpkg"), append = FALSE)
```
