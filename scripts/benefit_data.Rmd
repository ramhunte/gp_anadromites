...
flood_cost_ben <- flood_cost_ben %>%
  mutate(ratio_n = NA) %>% 
  mutate(ratio_n = ifelse(!is.na(avg_cost), avg_cost / n, NA))
flood_cost_ben$ratio <- replace(flood_cost_ben$ratio, is.infinite(flood_cost_ben$ratio), 0)
flood_cost_ben <- flood_cost_ben %>%
  mutate(ratio_n_log = if_else(!is.na(ratio_n) & ratio != 0, log(ratio_n), NA_real_))

#ELJ
elj_cost_ben$ratio_n<-elj_cost_ben$avg_cost/elj_cost_ben$n
...
ggplot() +
  ...
  theme_minimal()
```
## ELJ figure highlighting top 5 subbasin
```{r}
elj_cost_ben_top5<- elj_cost_ben %>% 
  arrange(desc(ratio_n)) %>% 
  slice_head(n = 5)
ggplot() +
  geom_sf(data = subs_stl, aes()) + #all subbasins
