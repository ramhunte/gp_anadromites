...
average_elj <- elj_cost %>% 
  select(noaa_subba, length, energy, relative_cost, lower_cost_estimate, upper_cost_estimate, final_lower_cost, final_upper_cost) %>% 
  mutate(avg_per_mile =  ifelse(
        ...
        # first, we have total cost per stream reach. We take the average of the low and high estimates and divide it by the lenght (m) and converted to miles to get cost per mile
        (final_lower_cost + final_upper_cost/2)/(length * 0.000621371),
        # low relative costs and energy streams are already in $/mile, so we just take the avg
        lower_cost_estimate + upper_cost_estimate/2
        ))
