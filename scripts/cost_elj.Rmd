...
# sf::st_write(flow_stl, here("data", "HARP", "flow_stl_mod.shp"), row.names=FALSE, append = FALSE)
```
...
#this doesn't load correctly yet
```{r}
#convert to sf for mapping purposes
elj_no_inf_cost_sf <- st_as_sf(elj_no_inf_cost)
 ggplot() +
  geom_sf(data = subs_stl, aes()) +
  geom_sf(data = elj_no_inf_cost_sf, aes(fill = avg_cost)) +
  scale_fill_viridis_b(breaks = c(0, 1500, 4500, 7000),
                       #labels = c("0", "$4500", "$7000")
                       ) +
