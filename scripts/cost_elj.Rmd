...
elj_no_inf_cost$length_mi<-elj_no_inf_cost$length*0.000621371
#multiply miles by $/mile to get total dollars
elj_no_inf_cost$final_lower_cost<-elj_no_inf_cost$lower_cost_estimate*elj_no_inf_cost$length_mi
elj_no_inf_cost$final_upper_cost<-elj_no_inf_cost$upper_cost_estimate*elj_no_inf_cost$length_mi
elj_no_inf_cost$avg_cost<-(elj_no_inf_cost$final_lower_cost+elj_no_inf_cost$final_upper_cost)/2
```

## MAP THE RESULTS
```{r}
#convert to sf for mapping purposes
elj_no_inf_cost_sf <- st_as_sf(elj_no_inf_cost)
ggplot() +
  geom_sf(data = subs_stl, aes()) +
  geom_sf(data = elj_no_inf_cost_sf, aes(color = avg_cost)) + #use color, not fill bc line
  scale_color_viridis_c(breaks = c(0, 1500, 4500, 7000), option = "E", name = "Average Cost ($)", direction=-1)+
  theme_minimal()
```
...
elj_no_inf_cost_sf$subbasin_name <- str_replace_all(elj_no_inf_cost_sf$noaa_subba, "_", " ") 
elj_no_inf_cost_sf$subbasin_name <-str_replace_all(elj_no_inf_cost_sf$subbasin_name, "Stillaguamish", "")
elj_no_inf_cost_sf$subbasin_name <-str_replace_all(elj_no_inf_cost_sf$subbasin_name, "mainstem", "Mainstem")

#create a summary table that has the lower, upper and average cost by subbasin
elj_summary<-elj_no_inf_cost_sf %>% group_by(subbasin_name) %>% 
  summarise(
...
ggplot(elj_summary, aes(x = subbasin_name)) +
  geom_errorbar(
    aes(ymin = sum_final_lower_cost/1000, ymax = sum_final_upper_cost/1000),
    position = "dodge",
    width = 0.2
  ) +
  geom_point(aes(y = sum_avg_cost/1000), position = "dodge", size = 3) +
  labs(title = "Costs per Subbasin in the Stillaguamish",
       x = "Subbasin",
       y = "Cost (Thousands of Dollars)") +
  theme_minimal()+
