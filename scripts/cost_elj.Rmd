...
elj_cost <- elj_cost %>%
  rowwise() %>%
  mutate(
    final_lower_cost = ifelse((relative_cost == 'low' && energy == 'low') || 
                                (relative_cost == 'medium' && energy == 'low') || 
                                (relative_cost == 'low' && energy == 'medium'),
                              lower_cost_estimate * length * 0.000621371 * 0.5,
                              lower_cost_estimate * length * ifelse(energy == 'low', 1/100, 
                              ifelse(energy == 'medium', 1/200, ifelse(energy == 'high', 1/400, 1)))
    ),
...
# find avg cost 
```{r}
average_elj <- elj_cost %>% 
  select(noaa_subba, energy, relative_cost, lower_cost_estimate, upper_cost_estimate) %>% 
  mutate(avg_cost_estimate = (lower_cost_estimate + upper_cost_estimate)/2,
         metric =  ifelse(
        (relative_cost == "medium" & energy == "medium") |
        (relative_cost == "medium" & energy == "high") |
        (relative_cost == "high" & energy == "medium") |
        (relative_cost == "high" & energy == "low") |
        (relative_cost == "low" & energy == "high") |
        (relative_cost == "high" & energy == "high"),
        "per_structure",
        # low relative costs and energy streams are already in $/mile, so we just take the avg
        "per_mile"
        ),
        hab_unit = "elj"
        ) 

# creating a summary table of elj avg estiamtes by basin
avg_elj_cost_summary <- average_elj %>% 
  rename(Subbasin = noaa_subba,
         Restoration = hab_unit,
         Metric = metric) %>% 
  group_by(Subbasin, Restoration, Metric) %>% 
  summarise("Lower Average Cost" = paste("$", format(round(mean(lower_cost_estimate)), big.mark = ",")),
            "Upper Average Cost" = paste("$", format(round(mean(upper_cost_estimate)), big.mark = ",")))

avg_elj_cost_summary %>%  
kableExtra::kable() %>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>% 
   kableExtra::save_kable(file = here("final_prods","tables","average_elj_costs.png"), zoom = 4)

write_csv(avg_elj_cost_summary, here("final_prods", "tables", "average_elj_costs.csv"), append = FALSE)
