...
```{r}
demo_layer<-st_join(POC_stilly, Poverty_stilly)
demo_layer<-st_join(demo_layer, Unemployment_stilly)
demo_layer<-st_join(demo_layer, tribal_jurisdiction_stilly) 
demo_layer<-demo_layer %>% select(census_tra.x, noaa_subba.x, percent_POC, percent_pov, percent_unemp, tribal_lands) %>% rename("census_tract"="census_tra.x") %>% 
  rename("noaa_subba"="noaa_subba.x")
...
demo_layer_filtered <- demo_layer %>%
  mutate(POC_score = ifelse(percent_POC > 0.125, 1, 0),
         Poverty_score = ifelse(percent_pov > 0.16, 1, 0),
         Unemployed_score = ifelse(percent_unemp > 0.042, 1, 0),
         Tribal_score= ifelse(tribal_lands %in% 1, 1, 0),
         Total_score=POC_score+Poverty_score+Unemployed_score+Tribal_score
         ) %>%
  st_drop_geometry() %>% 
  group_by(noaa_subba) %>% 
  mutate(Subba_POC_score=mean(POC_score),
         Subba_Poverty_score=mean(Poverty_score),
         Subba_Unemployed_score=mean(Unemployed_score),
         Subba_Tribal_Score=mean(Tribal_score),
         Subba_Total_Score=mean(Total_score)) %>% 
...
```{r}
ggmap::ggmap(basemap) +
    # subbasins with score info
  geom_sf(data = filter(demo_layer_filtered), 
          aes(fill = Total_score),
          inherit.aes = FALSE) +
  # #basin
